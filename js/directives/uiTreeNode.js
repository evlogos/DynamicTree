                ...
                $q.when(scope.$treeScope.$callbacks.beforeDrop(dragEventArgs))
                    ...
                    .then(function (allowDrop) {
                      if (allowDrop !== false && scope.$$allowNodeDrop && !outOfBounds
                       && !dragEventArgs.source.nodeScope.$treeScope.editDisabled
                          && dragEventArgs.source.nodeScope.$modelValue.value>0
                      ) { // node drop accepted)
                        dragInfo.apply();
...
angular.module('ui.tree').directive('checkPrime', function() {
  return {
    require: 'ngModel',
